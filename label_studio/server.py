...
def _create_user(input_args, config):
    ...
    if not org:
        org = Organization.create_organization(
            created_by=user, title='Yamato', legacy_api_tokens_enabled=input_args.enable_legacy_api_token
        )
    else:
        org.add_user(user)
...
def main():
    ...
    # init
        ...
        print('')
        print('Yamato has been successfully initialized.')
        if input_args.command != 'start' and input_args.project_name:
            print('Start the server: yamato start ' + input_args.project_name)
            return
        ...
        if input_args.project_name and not _project_exists(input_args.project_name):
            ...
            if project_path.exists():
                ...
                print(
                    Fore.LIGHTYELLOW_EX
                    + '\n*** WARNING! ***\n'
                    + f'Project {input_args.project_name} migrated to Yamato Database\n'
    ...
    # on `start` command, launch browser if --no-browser is not specified and start Yamato server
    if input_args.command == 'start' or input_args.command is None:
        ...
        if cert_file or key_file:
            logger.error(
                "Yamato doesn't support SSL web server with cert and key.\nUse nginx or other servers for it."
            )
            return
